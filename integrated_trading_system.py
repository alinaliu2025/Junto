"""
Integrated HRM Jet Signal + Z-Score Strikemap Trading System
Combines corporate aviation intelligence with options statistical arbitrage
"""
...
class ZScoreStrikemap:
    ...
    def __init__(self):
        ...
        self.relaxed_spread_pct = 40  # For puts if no liquid candidates
        self.relaxed_volume = 20
        self.relaxed_oi = 50
    ...
    def get_option_chain(self, ticker: str, expiry_date: str = None) -> pd.DataFrame:
        ...
        In production, would use real options data API
        """
        try:
            ...
            # In production, use real options data from Polygon, Tradier, etc.
            strikes = np.arange(
                current_price * 0.8, 
                current_price * 1.2, 
                current_price * 0.02
            ...
            for strike in strikes:
                # Calculate moneyness and basic Greeks (simplified)
                moneyness = strike / current_price
                days_to_expiry = 30  # Assume 30 DTE for demo
                
                # Synthetic option pricing (simplified Black-Scholes approximation)
                ...
                call_iv = 0.25 + abs(moneyness - 1) * 0.1  # IV skew
    ...
    def preprocess_options(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Remove wide spreads
        df = df[df['Spread_Pct'] <= 60]  # Initial filter
    ...
    def fit_regression_model(self, df: pd.DataFrame, underlying_move: float) -> pd.DataFrame:
        ...
        # Simplified: use delta * underlying_move as baseline expectation
        y_expected = df['Delta'] * underlying_move * df['Current_Price']
        
        # Actual price change (synthetic for demo)
        # In production, this would be actual observed price changes
    ...
    def screen_for_trades(self, df: pd.DataFrame) -> dict:
        ...
        # Relaxed filters for puts if no candidates
        if liquid_puts.empty:
            liquid_puts = df[
                (df['Type'] == 'Put') &
                (df['Spread_Pct'] <= self.relaxed_spread_pct) &
                (df['Volume'] >= self.relaxed_volume) &
                (df['Open_Interest'] >= self.relaxed_oi)
            ].copy()
        ...
        # Overvalued (potential shorts, but risky)
        overvalued_calls = atm_otm_calls[atm_otm_calls['Z_Score'] > 1.5].sort_values('Z_Score', ascending=False)
        overvalued_puts = atm_otm_puts[atm_otm_puts['Z_Score'] > 1.5].sort_values('Z_Score', ascending=False)
        
        return {
            'undervalued_calls': undervalued_calls.head(5),
            'undervalued_puts': undervalued_puts.head(5),
            'overvalued_calls': overvalued_calls.head(3),
            'overvalued_puts': overvalued_puts.head(3),
